...
from utilities.configs import DATASET_DIR, GALLERY_MASTER_PASSWORD
...
def show_gallery(person_name: str = "", correct_password: str = GALLERY_MASTER_PASSWORD, allow_removal: bool = False):
    ...
    for person in persons_folders:
        if not person_name or person_name.lower() == person.lower():
            _display_content(person, show_expanded, allow_removal)

def _display_content(person_name: str = "", show_expanded: bool = False, allow_removal: bool = False) -> None:
    ...
    with st.expander(f"Gallery for '{person_name}' ({len(person_images)} images)", expanded=show_expanded):
        ...
        with gallery_items_placeholder:
            _display_images(person_images, person_name, allow_removal=allow_removal)

def _display_images(images_names: list[str], person_name: str, max_columns: int = 4, allow_removal: bool = False) -> None:
